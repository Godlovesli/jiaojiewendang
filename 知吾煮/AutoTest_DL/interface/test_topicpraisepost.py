...
class topicpraisepostTest(MyTest):
    '''话题点赞/取消点赞'''
    url_path = '/topic/praise/post'
    ...
    def test_praise_cpsuccess(self):
        ...
        if data_count > 0:
            ...
            r = self.myhttp('POST',
                            self.url_path,
                            params,
                            # {'topicId': topicId, 'praise': 'cancelPraise'},
                            token
                            )
            print r
            js = json.loads(r)
    ...
    def test_praise_yz(self):
        ...
        sql = "select mipot_topic.id from mipot_topic,mipot_topic_praise  \
                where mipot_topic.id=mipot_topic_praise.topic_id  and  \
                mipot_topic.is_deleted='0' and mipot_topic_praise.user_id='1053' "
        data_count = cursor.execute(sql)
        print data_count
        if data_count > 0:
            rows = cursor.fetchone()
    ...
    def test_praise_signerror(self):
        ...
        # params = urllib.urlencode({'topicId': 16, 'praise': 'praise'})
