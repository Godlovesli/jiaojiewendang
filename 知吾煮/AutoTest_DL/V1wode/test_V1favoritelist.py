...
#
# class v1minefavoritelistTest(MyTest):
#     '''收藏列表'''
#     url_path = '/v1/mine/favorite/list'
#
#     @classmethod
#     def setUpClass(cls):
#         pass
#
#     def test_favoritelist_success(self):
#         '''所有参数都传'''
...
#
#     def test_favoritelist_success1(self):
#         '''所有参数都不传'''
#         token = Login().login()  # 引用登录
#         params = ''
#         print params
...
#         self.assertIn('获取我发布的成功', js['message'])
#
#     def test_favoritelist_signerror(self):
#         '''sign不正确'''
#         token = Login().login()
...
#
#     def test_favoritelist_noncerror(self):
#         '''nonce不正确'''
#         token = Login().login()
#         self.url = self.base_url + self.url_path
#         self.signature = generateSignature(self.nonce, "GET", self.url)
